                ...
                <template v-for="(model,index) in dataModel" v-if="canEdit(model.edit)">
                    <el-col :span="24" v-if="model.type == 'text'">
                        <el-form-item :label="model.name" label-width="100px" :prop="model.field">
                            <slot :name="model.field">
                                <el-input v-model="ruleForm[model.field]" :placeholder="model.placeholder" auto-complete="off"></el-input>
                            </slot>
                        </el-form-item>
                    </el-col>
                    <el-col :span="24" v-if="model.type == 'textarea'">
                        <el-form-item :label="model.name" label-width="100px" :prop="model.field">
                            <slot :name="model.field">
                                <el-input type="textarea" v-model="ruleForm[model.field]" auto-complete="off"></el-input>
                            </slot>
                        </el-form-item>
                    </el-col>
                    <el-col :span="24" v-if="model.type == 'date'">
                        <el-form-item :label="model.name" label-width="100px" :prop="model.field">
                            <slot :name="model.field">
                                <el-date-picker :type="model.dateType" :placeholder="model.placeholder" v-model="ruleForm[model.field]"></el-date-picker>
                            </slot>
                        </el-form-item>
                    </el-col>
            ...
            confirmAction (formName) {
                this.$refs.ruleForm.validate((valid) => {
                    if (valid) {
                        ...
                        this.$message.error({message: '表单有误，请检查再提交', duration: 5000})
                        return false;
                    }
                });
            },
